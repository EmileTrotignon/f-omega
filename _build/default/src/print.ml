...
let svar n = !^ (string_of_svar n)

let dvar id a =
  let n = svar (Syntax.svar a.name) in
  if id > 0 then n ^^ P.int id
  else if id < 0 then n ^^ !^"/" ^^ (P.int (- id))
  else n

let evar (x : evar) = !^ (string_of_evar x)
...
let cvar a =
  if debugging_print then
    let rec cvar a =
      let n = svar (Syntax.svar a.name) in
      let n =
      if a.id > 0 then n (* ^^ !^"!"*) ^^ P.backslash ^^ P.int a.id else n in
      let n = if a.unifiable then P.char '%' ^^ n else n in
      match a.def with
      | Some {typ=t;_} -> P.(parens (n ^^ space ^^ equals ^^ space ^^ typ cvar t))
      | None -> n
    in
    cvar a
  else
