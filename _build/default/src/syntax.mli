...
(** The representation of variable is an important design choice.
   ...
   I chose to make them abstract to prevent me from mixing them up. *)
...
(** To allow for further changes of the type evar, we use smart
   ...
   from a name. Both functions are currently the identity. *)

val evar : string -> evar

val svar : string -> svar

(** They also get destructors for debugging purposes *)

val string_of_evar : evar -> string

val string_of_svar : svar -> string
...
(** Declarations are either expression bindings or type declarations.  Type
   ...
   new abstract type---which will be used to model type generativity.  *)
type decl = decl_ loc
...
type cvar = {name: string; id: int; def: def option; unifiable : bool}
...
val cvar : ?id:int -> ?def:def -> ?unifiable:bool -> svar -> cvar
